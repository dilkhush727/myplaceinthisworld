...
<div class="container pb-5">
    ...
    <h1 class="mb-5 pb-5">{{ t('divisions.title', 'Divisions of Learning') }}</h1>
    ...
    <div class="row g-4 justify-content-center dol-v2 mt-5">
            ...
            <div class="card h-100 dol-card dol-primary">
                ...
                <div class="card-body d-flex flex-column dol-body">
                    <div class="text-center mb-2">
                        <h4 class="card-title"><strong>{{ t('divisions.primary', 'Primary') }}</strong></h4>
                        <h5>{{ t('divisions.primary_grades', 'Grade (1-3)') }}</h5>
                    </div>

                    <p class="card-text flex-grow-1">
                        {{ t('divisions.primary_desc', 'Primary students will become familiar with some African Kings and Queens. The hands-on activities will engage students as they begin to learn about African heritage through the lens of African Kings and Queens. This curriculum will also give students a sense of connectedness, create a new understanding of Black people and culture, and develop a sense of belonging, especially for Black students.') }}
                    </p>
                    ...
                    @if($hasPrimary)
                        ...
                        <a href="{{ route('divisions.primary') }}" class="btn btn-success">
                            {{ t('divisions.view_primary', 'View Primary Resources') }}
                        </a>
                    @else
                        <span class="badge bg-danger">{{ t('common.locked', 'Locked') }}</span>

                        @if(auth()->user()->hasRole('school'))
                            <a href="{{ route('school.memberships.manage') }}" class="btn btn-outline-danger">
                                {{ t('divisions.unlock_primary', 'Unlock Primary') }}
                            </a>
                        @else
                            <button class="btn btn-outline-secondary" disabled>
                                {{ t('divisions.contact_school', 'Contact your school to access') }}
                            </button>
                        @endif
                    @endif
        ...
        <div class="col-md-4">
            <div class="card h-100 dol-card dol-junior">
                ...
                <div class="card-body d-flex flex-column dol-body">
                    <div class="text-center mb-2">
                        <h4 class="card-title"><strong>{{ t('divisions.ji', 'Junior/Intermediate') }}</strong></h4>
                        <h5>{{ t('divisions.ji_grades', 'Grade (4-6)') }}</h5>
                    </div>

                    <p class="card-text flex-grow-1">
                        {{ t('divisions.ji_desc', 'As Junior/Intermediate students get to know the African Kings and Queens presented, the hands-on activities will engage them. The familiar learning platforms and the use of social media tools will keep them motivated. This curriculum will also give students a sense of connectedness, create a new understanding of Black people and culture, and develop a sense of belonging, especially for Black students.') }}
                    </p>
                    ...
                    @if($hasJi)
                        ...
                        <a href="{{ route('divisions.ji') }}" class="btn btn-success">
                            {{ t('divisions.view_ji', 'View JI Resources') }}
                        </a>
                    @else
                        <span class="badge bg-danger">{{ t('common.locked', 'Locked') }}</span>
                        ...
                        @if(auth()->user()->hasRole('school'))
                            <a href="{{ route('school.memberships.manage') }}" class="btn btn-outline-danger">
                                {{ t('divisions.unlock_ji', 'Unlock JI') }}
                            </a>
                        @else
                            <button class="btn btn-outline-secondary" disabled>
                                {{ t('divisions.contact_school', 'Contact your school to access') }}
                ...
                <div class="card-body d-flex flex-column dol-body">
                    ...
                    @if($hasHs)
                        <span class="badge bg-success">
                            {{ t('divisions.active_included', 'Active (Included)') }}
                        </span>

                        <a href="{{ route('divisions.highschool') }}" class="btn btn-success">
                            {{ t('divisions.view_highschool', 'View High School Resources') }}
                        </a>
                    @else
                        <span class="badge bg-danger mb-2">
                            {{ t('divisions.not_active', 'Unexpected: Not Active') }}
                        </span>
